...
int main()
{
	...
	while (true) {
		...
		if (runningCase == 41) {
			...
			if (runLenth > 2160)TargetSpeed = 100;//原来是50，避障，60可行  ***这里改成60或者70有很多提高
			...
			if (followingFlag) {
				...
				if (!end)
				{
					if (runningCase == 41)
					{
						...
						if (TargetSpeed == 60)
						{
							file << "T60小车速度: " << mainVehicleSpeed << " m/s" << std::endl;
							file << "T60小车位置：" << mainVehiclePos.x << ", " << mainVehiclePos.y << endl;
							if (mainVehicleSpeed > 8.33)
							{
								file << "T60超速！" << endl;
							}
